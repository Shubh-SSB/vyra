...
import { ArrowLeft, Check, MessageSquare, Users, ShieldAlert, Loader2 } from "lucide-react";
...
const privacyOptions = [
  {
    id: "EVERYONE",
    title: "Everyone",
    description: "Anyone can start a conversation and send you direct messages.",
    icon: <MessageSquare className="h-4 w-4" />,
  },
  {
    id: "FRIENDS_ONLY",
    title: "Friends Only",
    description: "Only accepted friends can start conversations and send you direct messages.",
    icon: <Users className="h-4 w-4" />,
  },
  {
    id: "NOBODY",
    title: "Nobody",
    description: "No one can start new conversations with you. You must initiate them.",
...
export default function MessagePrivacyPage() {
  ...
  const currentPrivacy = user?.messagePrivacy || "EVERYONE";

  const [selectedPrivacy, setSelectedPrivacy] = useState<string>(currentPrivacy);
  ...
  const handleSelect = (optionId: string) => {
    ...
    updatePrivacy(
      {
        messagePrivacy: optionId,
      },
      {
        onSuccess: () => {
          enqueueSnackbar("Message privacy updated successfully", { variant: "success" });
        },
        onError: (err: any) => {
          setSelectedPrivacy(previousValue);
          enqueueSnackbar(err?.response?.data?.message || "Failed to update message privacy", { variant: "error" });
        },
      }
    );
  };
  ...
  return (
    <div className="flex h-svh">
      <SettingSidebar name="Message Privacy" navigateTo="Back to Settings" path="/settings" tagline="This page will help you manage who can send you direct messages." />
        ...
        <div className="mx-auto max-w-[620px] lg:max-w-[1024px] w-full">
          ...
          <div className="space-y-6">
            <div className="rounded-3xl border border-white/[0.06] bg-[#151517]/85 p-6 md:p-8 shadow-xl">
              {/* Heading */}
              <div className="mb-6">
                <h2 className="text-lg font-bold tracking-tight text-[#eeece4]">Direct Messages</h2>
                <p className="text-xs text-muted-foreground mt-1">Control who can initiate direct message conversations with you.</p>
              </div>
              ...
              <p className="mt-4 px-4 text-[10px] text-muted-foreground/50 leading-relaxed text-center">
                Note: This setting does not affect existing active conversations or group conversations.
              </p>
            </div>
          </div>
